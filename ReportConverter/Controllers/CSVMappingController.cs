...
{
    public class CSVMappingController : Controller
    {
        //
        // GET: /CSVMapping/
        ...
        public ActionResult Index(HttpPostedFileBase postedFile, BigViewModel bigModel)
        {
            ...
            if (postedFile != null)
            {
                ...
                string[] resultArray = result.Split(new string[] { NewLineSeperator }, StringSplitOptions.None).Take(5).ToArray();
        ...
        public string[][] CreateMatrix(string[] Array, string elementSeparator, string subelementSeparator)
        {
            ...
            for (int i = 0; i < Array.Length; i++)
            {
                ...
                /*
                Commented code for subelement separator
                 */
            ...
            string[][] jaggedArray_Transpose = CreateTranspose(jaggedArray);

            //return jaggedArray;

            return jaggedArray_Transpose;
        }

        public string[][] CreateTranspose(string[][] jaggedArray)
        {
            string[][] jaggedArray_Transpose = new string[jaggedArray[0].Length][];

            List<string> LabelList = new List<string> { };



            for (int i = 0; i < jaggedArray_Transpose.Length; i++)
            {
                LabelList.Clear();
                for (int j = 0; j < jaggedArray.Length; j++)
                {
                    //LabelArray[j] = jaggedArray[j][i];
                    LabelList.Add(jaggedArray[j][i]);
                }

                /*
                 * Allocating array replaces values of all previous elements - why ?
                 */

                //jaggedArray_Transpose[i] = LabelArray;
                jaggedArray_Transpose[i] = LabelList.ToArray();
            }
        ...
        public void xEditable(string clicked_id, string clicked_parentid, string value)
        {
            ...
            /*
             If(IsheaderRowPresent == false)
              {
            SegmentInitiator.Add(clicked_parentid);
              }
            */
